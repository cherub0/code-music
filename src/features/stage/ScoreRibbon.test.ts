...
describe('maximumWindowDemand', () => {
  it('keeps capacity for every note in the densest inclusive visible window', () => {
    ...
    expect(maximumWindowDemand(scoreWithStarts(Array.from({ length: 21 }, (_, index) => index)))).toBe(18);
  });

  it('uses a smaller note window for low preview quality', () => {
    const score = scoreWithStarts(Array.from({ length: 21 }, (_, index) => index));

    expect(maximumWindowDemand(score, 8)).toBe(18);
    expect(maximumWindowDemand(score, 4)).toBe(10);
  });

  it('allocates dense-window capacity for sustained notes that started much earlier', () => {
    const score = scoreWithStarts([0, ...Array.from({ length: 600 }, () => 50)]);
    score.notes[0].durationSeconds = 100;

    expect(maximumWindowDemand(score, 0)).toBe(601);
  });
});
